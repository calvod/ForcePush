        ...
        # print numberOfBlobs
                        ...
                        if len(contours) > 0:
                                ...
                                #make sure the contour is large enough to even bother with
                                    if area > 200:
                                        #print "area greater than 200"
                                        #make sure the contour is square enough
                                        x,y,w,h = cv2.boundingRect(contours[i])
                                        # print float(w) / float(h)
                                        # print float(h) / float(w)
                                        if float(w) / float(h) > 0.005 and float(w) / float(h) < 5 and float(h) / float(w) > 0.005 and float(h) / float(w) < 5:
                                                if area > largestArea:
                                                        largestArea = area
                                                        bestContour = i
                                ...
                                if bestContour >= 0:
                                        ...
                                        print "found green screen in image: " + fn
                                        #cv2.imwrite( "CVOutput/thresholdedImage" + fn, resizedInputPhoto)
                                        #cv2.imshow('contours', resizedInputPhoto)
                                        #cv2.waitKey(0)
                                        currentScore += 100
                                else:
                                        print "found contours, but none that looked like a green screen in image: " + fn
                                        #cv2.imwrite( "CVOutput/thresholdedImage" + fn, resizedInputPhoto) 
                                        #cv2.imshow('contours', resizedInputPhoto)
                                        #cv2.waitKey(0)
